...
def tal(tal_str):
    ...
    stream.
    '''
    exp = b'(?P<onset>[+\-]\d+(?:\.\d*)?)(?:\x15(?P<duration>\d+(?:\.\d*)?))?(\x14(?P<annotation>[^\x00]*))?(?:\x14\x00)'

    def annotation_to_list(annotation):
        temp = annotation.split(b'\x14') if annotation else []
        return [x.decode('utf-8') for x in temp]
...
def edf_header(f):
    ...
    (day, month, year) = [int(x)
                          for x in re.findall('(\d+)', f.read(8).decode('utf-8'))]
    (hour, minute, sec) = [int(x)
                           for x in re.findall('(\d+)', f.read(8).decode('utf-8'))]
...
class BaseEDFReader:
    ...
    def read_header(self):
        self.header = edf_header(self.file)

        # calculate ranges for rescaling
        self.dig_min = self.header['digital_min']
        self.phys_min = self.header['physical_min']
        phys_range = self.header['physical_max'] - self.header['physical_min']
        dig_range = self.header['digital_max'] - self.header['digital_min']
    ...
    def convert_record(self, raw_record):
        ...
        for (i, samples) in enumerate(raw_record):
            if header['label'][i].decode('utf-8') == EVENT_CHANNEL:
                ann = tal(samples)
                time = ann[0][0]
                events.extend(ann[1:])
                # print(i, samples)
